...
export const fetchLanguages = createAsyncThunk('languages/getLanguages', async (locale: string) => {
  const response = await axios.get<LanguageData>(`${DATABASE_PATH}/languages${locale === 'en' ? '' : '-' + locale}`);
